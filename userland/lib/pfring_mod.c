...
/*
#define MAX_NUM_PIDS 131073 ///proc/sys/kernel/pid_max
#define MAX_NUM_RINGS 25
//队列的结点结构
typedef struct Node{
  pfring *data;
  struct Node *next;
...
void insertQueue(LinkQueue *queue,pfring *temp){
  printf("insertQueue %p %d\n",temp->buffer,temp->get_ring_id(temp));
...
//最后一个元素
pfring *lastQueue(LinkQueue queue){
    int i = 1;
    Queue q = queue.front->next;
    Queue tmp=q;
    while (q != NULL) {
...
/*int queuenum=0,pidnum=0;
int pids[MAX_NUM_PIDS];
LinkQueue queue[MAX_NUM_PIDS];
pfring *ring_list[MAX_NUM_PIDS][256];*/
...
int pfring_mod_open_setup(pfring *ring) {
  ...
  pfring_hw_ft_init(ring);//problem,???
  ...
  //ADD
  int pid=getpid();
  if(pids[pid]==0){//one new process
    ring_list[pid][pids[pid]]=(pfring *)malloc(sizeof(pfring));
    *ring_list[pid][pids[pid]]=*ring;
    initQueue(&queue[pid]);
    pids[pid]++;
  ...
  printf("pfring_mod_open_setup %p\n",ring_list[getpid()][pids[getpid()]-1]->buffer);
...
int pfring_mod_open(pfring *ring) {
  ...
  int pid=getpid();
  if(pids[pid]==0){//one new process
    printf("one new process initQueue\n");
    //ring_list[pid][pids[pid]]=(pfring *)malloc(sizeof(pfring));
    //*ring_list[pid][pids[pid]]=*ring;
    initQueue(&queue[pid]);
  }else{//one old process
    printf("one old process\n");
    //ring_list[pid][pids[pid]]=(pfring *)malloc(sizeof(pfring));
    //*ring_list[pid][pids[pid]]=*ring;
  }
...
u_int8_t pfring_mod_get_num_rx_channels(pfring *ring) {
  ...
  printf("SO_GET_NUM_RX_CHANNELS :%d \n",rc);
...
int pfring_mod_stats(pfring *ring, pfring_stat *stats) {
  ...
  if((ring->slots_info != NULL) && (stats != NULL)) {
    ...
    for(i=0;i<pids[pid];i++){
      stats->recv+=ring_list[pid][i]->slots_info->tot_read;
      stats->drop+=ring_list[pid][i]->slots_info->tot_lost;
      printf("i=%d,buffer:%p,recv:%d,drop:%d\n",i,ring_list[pid][i]->buffer,stats->recv,stats->drop);
    }
    return(0);
  }
...
int pfring_mod_recv(pfring *ring, u_char** buffer, u_int buffer_len,
  ...
  do_pfring_recv:
    ...
    if(pfring_there_is_pkt_available(ring)) {
      //ADD
      printf("pfring_there_is_pkt_available %p\n",ring->buffer);
	...
	*buffer = (u_char *) &bucket[ring->slot_header_len];
    ...
    printf("nothing to do,wait,sizequeue:%d buffer:%p \n",sizeQueue(queue[pid]),ring->buffer);

    if(sizeQueue(queue[pid])>1){//this process has more than one ring to be read
      printf("sizeQueue(queue[pid])>1 %p %p\n",queue[pid].front->next->next->data->buffer,ring->buffer);
      return(100);
    }
    ...
    if(wait_for_incoming_packet) {//1 ./pfcount -a ... =0
...
u_int32_t pfring_mod_get_ring_id(pfring *ring) {
  ...
  printf("pfring_mod_get_ring_id:rc:%d,fd:%d,id:%d\n",rc,ring->fd,id);
  return((rc == 0) ? id : -1);
}
...
int pfring_mod_enable_ring(pfring *ring) {
  //ADD
  printf("pfring_mod_enable_ring,%p\n",ring->buffer);
