...
{
  if(pfr==NULL){
    printk("check_free_ring_slot NULL\n");
    return (0);
  }
  printk("check_free_ring_slot %p\n",pfr->ring_memory);
...
static void ring_proc_add(struct pf_ring_socket *pfr)
{
  ...
  if(netns != NULL &&
    ...
    printk("here ring_proc_add success %s %p %d\n",pfr->sock_proc_name,pfr->ring_memory,pfr->ring_id);
...
static void ring_proc_remove(struct pf_ring_socket *pfr)
{
  ...
  if(netns != NULL &&
    ...
    printk("Removed /proc/net/pf_ring/%s\n", pfr->sock_proc_name);
...
static inline void ring_remove(struct sock *sk_to_delete)
{
  ...
  printk("ring_remove:%p\n",pfr_to_delete->ring_memory);
				    ...
				    void *raw_data, uint raw_data_len)
{
  ...
  if(pfr->slots_info->tot_pkts%20==0){
    //ADD
    struct sock *sk;
    u_int32_t last_list_idx;
    int flag=0;
    sk = (struct sock*)lockless_list_get_first(&ring_table, &last_list_idx);
    struct pf_ring_socket *pfr0;
    while(flag==0 && sk!=NULL){//find ring to write
      pfr0=ring_sk(sk);
      printk("pfr0:%p pfr:%p\n",pfr0->ring_memory,pfr->ring_memory);
      if(pfr0==NULL){
        printk("pfr0=NULL \n");
        break;
      }
      else if(pfr0!=NULL && pfr0->ring_memory!=NULL && pfr0->ring_pid==pfr->ring_pid && pfr0->ring_memory!=pfr->ring_memory){//not now ring
        if(check_free_ring_slot(pfr0)){//this new ring has free space
          printk("pfr0 has free space :%p\n",pfr0->ring_memory);
          pfr->iswriting=0;//set now ring not writing
          pfr=ring_sk(sk);//set pfr=new ring
          pfr->iswriting=1;//set new ring writing
          //send ring_memory into queue
          struct sk_buff *nl_skb;
          struct nlmsghdr *nlh;
          int ret=0;
          char pbuf[20];
          sprintf(pbuf, "%d-%d", pfr->ring_pid, pfr->ring_id);
          //snprintf(pbuf, sizeof(pbuf),"%p",pfr->ring_memory);//make one new ring
          int len=strlen(pbuf);
          ...
          ret=netlink_unicast(nlsk,nl_skb,pfr->ring_pid,MSG_DONTWAIT);
          printk("1 after kernel send %s,full ring mem:%p %d\n",pbuf,pfr->ring_memory,pfr->ring_pid);
          printk("after %p %d\n",pfr->ring_memory,pfr->ring_active);
          flag=1;
        }else{
          printk("pfr0 is full\n");//new ring full,find the next
          sk = (struct sock*)lockless_list_get_next(&ring_table, &last_list_idx);
        }
      }else{//now ring,find the next
        printk("find the next\n");
        sk = (struct sock*)lockless_list_get_next(&ring_table, &last_list_idx);
      }
    }
    if(flag==0){//do not find the free other ring
      printk("not find target pfr0\n");
      struct sk_buff *nl_skb;
      struct nlmsghdr *nlh;
      int ret=0;
      char pbuf[20];
      snprintf(pbuf, sizeof(pbuf),"%s","create");//make one new ring
      int len=strlen(pbuf);
      nl_skb=nlmsg_new(len,GFP_ATOMIC);
      if(!nl_skb){
        printk("netlink alloc failure!\n");
      ...
      memcpy(nlmsg_data(nlh),pbuf,len);
      ret=netlink_unicast(nlsk,nl_skb,pfr->ring_pid,MSG_DONTWAIT);
      printk("2 after kernel send %s,full ring mem:%p %d\n",pbuf,pfr->ring_memory,pfr->ring_pid);
      pfr->iswriting=0;
    }
  }
  ...
  if(!check_free_ring_slot(pfr)) /* Full */ {
    ...
    printk("check_free_ring_slot full %p\n",pfr->ring_memory);
 ...
 //ADD
 printk("%p %d num_queued_pkts(pfr):%d pfr->poll_num_pkts_watermark:%d\n",pfr->ring_memory,pfr->slots_info->tot_pkts,num_queued_pkts(pfr), pfr->poll_num_pkts_watermark);
			   ...
			   u_int32_t num_rx_channels)
{
  ...
  */
  ...
  if((!pfring_enabled) || ((!pfr->ring_active) && (pfr->master_ring == NULL))){
    ...
    //ADD
    printk("add_skb_to_ring %p %d %d\n",pfr->ring_memory,pfring_enabled,pfr->ring_active);//ring_active=0
		       ...
		       )
{
  ...
  printk("ring_create success %p %d\n",pfr->ring_memory,pfr->ring_id);
...
{
  printk("ring_mmap start\n");
  struct sock *sk = sock->sk;
  struct pf_ring_socket *pfr = ring_sk(sk);
  int rc;
  unsigned long mem_id = vma->vm_pgoff; /* using vm_pgoff as memory id *///0
			   ...
			   int optlen)
{
  ...
  case SO_SET_POLL_WATERMARK:{
    ...
    char pbuf[20];
    sprintf(pbuf, "%d-%d", pfr->ring_pid, pfr->ring_id);
    //snprintf(pbuf, sizeof(pbuf),"%p",pfr->ring_memory);//get new ring,send ring_memory
    int len=strlen(pbuf);
    nl_skb=nlmsg_new(len,GFP_ATOMIC);
    if(!nl_skb){
      printk("netlink alloc failure!\n");
    ...
    memcpy(nlmsg_data(nlh),pbuf,len);
    ret=netlink_unicast(nlsk,nl_skb,pfr->ring_pid,MSG_DONTWAIT);
    printk("3 after kernel send,full ring mem:%s %p %d\n",pbuf,pfr->ring_memory,pfr->ring_pid);
			   ...
			   int __user *optlen)
{
  ...
  case SO_GET_RING_ID:
    printk("SO_GET_RING_ID:%d %d %d %p\n",len,sizeof(pfr->ring_id),pfr->ring_id,pfr->ring_memory);
  ...
  default:
    printk("ring_getsockopt default:%d\n",optname);
    return -ENOPROTOOPT;
  }

  if(put_user(len, optlen)){
    printk("ring_getsockopt put_user(len,optlen)\n");
    return(-EFAULT);
  }
  else
    return(0);
